...
public object UpdateInProductMessageSettings(string inProductMessageType)
{
    try
    {
        ...
        using (var cmd = new SqlCommand("UPDATE UserSettings SET Value=@val WHERE UserId=@uid AND Name='InProductMessageType'", conn))
        {
            ...
            cmd.Parameters.AddWithValue("@uid", Membership.GetUser()?.ProviderUserKey.ToString());
            conn.Open();
...
{
    string userId = Membership.GetUser()?.ProviderUserKey.ToString();
    return new { Success = true, Url = $"http://pageflakes.com/user/{userId}" };
}
[WebMethod]
[ScriptMethod(ResponseFormat = ResponseFormat.Json)]
...
{
    if (!int.TryParse(pageId, out int pid)) return new { Success = false, Error = "Invalid pageId" };
...
public object GetPageVersionNo(string pageId)
{
    ...
    using (var cmd = new SqlCommand("SELECT VersionNo FROM Pages WHERE PageId=@pid", conn))
    {
        cmd.Parameters.AddWithValue("@pid", pid);
        conn.Open();
        var version = cmd.ExecuteScalar()?.ToString() ?? "0";
...
public object RemoveModule(string id)
{
    string userId = Membership.GetUser()?.ProviderUserKey.ToString();
    if (string.IsNullOrEmpty(userId)) return new { Success = false, Error = "User not authenticated" };
    if (!int.TryParse(id, out int mid)) return new { Success = false, Error = "Invalid moduleId" };
    using (var conn = new SqlConnection(ConnectionString))
    {
        conn.Open();
        using (var tran = conn.BeginTransaction())
...
public object MoveModuleToPage(string moduleId, string fromPageId, string toPageId)
{
    ...
    if (string.IsNullOrEmpty(userId))
        return new { Success = false, Error = "User not authenticated" };
    if (!int.TryParse(moduleId, out int mid))
        return new { Success = false, Error = "Invalid moduleId" };
    if (!int.TryParse(fromPageId, out int fromPid))
        return new { Success = false, Error = "Invalid fromPageId" };
    if (!int.TryParse(toPageId, out int toPid))
...
{
    string userId = Membership.GetUser()?.ProviderUserKey.ToString();
    if (!int.TryParse(moduleId, out int mid)) return new { Success = false, Error = "Invalid moduleId" };
...
public object CloneModule(string moduleId, string targetPageId)
{
    string userId = Membership.GetUser()?.ProviderUserKey.ToString();
    if (!int.TryParse(moduleId, out int mid)) return new { Success = false, Error = "Invalid moduleId" };
    if (!int.TryParse(targetPageId, out int tpid)) return new { Success = false, Error = "Invalid targetPageId" };
...
public object GetUserProfileId(string pageId)
{
    ...
    if (!string.IsNullOrEmpty(pageId) && int.TryParse(pageId, out int pid))
    {
        using (var conn = new SqlConnection(ConnectionString))
    ...
    using (var cmd2 = new SqlCommand("SELECT ProfileId FROM UserProfiles WHERE UserId=@uid", conn2))
    {
        ...
        conn2.Open();
        var profileId = cmd2.ExecuteScalar()?.ToString();
...
public object GetAddFeedPopup(string version)
{
    try
    {
            ...
            "SELECT Html FROM UiTemplates WHERE Name='AddFeedPopup' AND (Version=@ver OR @ver IS NULL)", conn))
        {
            ...
            conn.Open();
            var html = cmd.ExecuteScalar()?.ToString();
        ...
        using (var tran = conn.BeginTransaction())
        {
            try
            {
                ...
                if (!string.IsNullOrWhiteSpace(emailTemplateName))
                {
                    ...
                    body = tmpl.ExecuteScalar()?.ToString();
                }
...
{
    string userId = Membership.GetUser()?.ProviderUserKey.ToString();
    if (!int.TryParse(bookmarkId, out int bid)) return new { Success = false, Error = "Invalid bookmarkId" };
...
public object GetAllBookmarksHtml()
{
    ...
    return new { Success = true, Html = $"<ul>{html}</ul>" };
}
[WebMethod]
[ScriptMethod(ResponseFormat = ResponseFormat.Json)]
...
{
    string userId = Membership.GetUser()?.ProviderUserKey.ToString();
    if (!int.TryParse(reminderId, out int rid)) return new { Success = false, Error = "Invalid reminderId" };
...
{
    string userId = Membership.GetUser()?.ProviderUserKey.ToString();
    if (!int.TryParse(themeId, out int tid)) return new { Success = false, Error = "Invalid themeId" };
