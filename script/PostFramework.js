...
AddContentWS.set_path("/Pageflakes/AddContentWS.asmx");
...
var SF = SecondaryFramework = {
    ...
    , firstVisitTooltip: function () {
        ...
        B.change_theme = function () { Start.toggleStart(false, 4) }; 
        B.change_layout = function () { Start.toggleStart(false, 5) }; 
        B.my_profile = function (E) { Start.toggleStart(false, 6, E) }; 
        var D = new Object(); 
        window.location.search.replace(new RegExp("([^?=&]+)(=([^&]*))?", "g"), function (F, E, H, G) { D[E] = G }); if (D.go != undefined) { if (B[D.go] != undefined) { B[D.go]() } else { if (D.go.toString().substring(0, 14) == "browse_flakes:") { B.browse_flakes(D.go.toString().substring(14, D.go.toString().length)) } else { if (D.go.toString().substring(0, 11) == "my_profile:") { B.my_profile(D.go.toString().substring(11, D.go.toString().length)) } } } }
    }, showWW: function () {
    ...
    , refreshChannel: function () {
        ...
        if (I > 1400) { 
            ...
            if (I > 1200) { 
                ...
                FeedViewer.ui_rss_reader = $(FeedViewer.RSS_READER_ID); 
                if (FeedViewer.ui_rss_reader == null) { 
                    var H = PU.getPosition($(FeedViewer.BODY_CONTAINER)); 
                    var D = App.createPopup(FeedViewer.RSS_READER_ID, "<b>" + Lang.RSS_READER + "</b>", FeedViewer.LEFT_POS, H[1] + FeedViewer.RIGHT_POS, FeedViewer.WIDTH, "auto"); 
                    FeedViewer.ui_rss_reader = $(FeedViewer.RSS_READER_ID); 
                    $ND(FeedViewer.ui_rss_reader); 
                    $("handleRssViewer").onmousedown = null; 
                    $("handleRssViewer").style.cursor = "auto"; 
                    var A = $("closeLinkRssViewer"); 
                    ...
                    var B = $("RssFeedReaderMain"); 
                    $D(B); 
                    D.innerHTML = ""; 
                    PU.changeParent("RssFeedReaderMain", D); 
                    ...
                    for (var C = 0; C < E.length; C++) { 
                        if (E[C].className == "flake_placeholder") { 
                            E[C].className = "" 
                        } 
                        if (E[C].className == "flake_footer") { 
                            E[C].className = ""; 
                            break 
                        } 
                    } 
                ...
                FeedViewer.vSpliter2.onDrag = function (J, N, M, L) { 
                    var K = FeedViewer.hSpliter.offsetLeft + FeedViewer.hSpliter.offsetWidth; 
                    FeedViewer.vSpliter2.style.left = K + "px"; 
                    if (N < FeedViewer.rssItemMinHeight) { 
                        FeedViewer.vSpliter2.style.top = FeedViewer.rssItemMinHeight + "px" 
                    } 
                    else { 
                        if (N > FeedViewer.rssItemMaxHeight) { 
                ...
                FeedViewer.vSpliter2.onDragStart = function (J, M, L, K) { 
                ...
                FeedViewer.vSpliter2.onDragEnd = function (J, L) { 
                    var K = FeedViewer.vSpliter2.style.top.substring(0, FeedViewer.vSpliter2.style.top.length - 2); 
                    FeedViewer.diff_y = K - FeedViewer.vSplitOldTop; 
                    FeedViewer.vSplitOldTop = FeedViewer.vSpliter2.style.top.substring(0, FeedViewer.vSpliter2.style.top.length - 2); 
                ...
                FeedViewer.hSpliter2.onDrag = function (J, M, L, K) { 
                    FeedViewer.hSpliter2.style.top = (FeedViewer.readingAreaTop - FeedViewer.VIEWER_TOP) + "px"; 
                    if (J < FeedViewer.channelListMinWidth) { 
                ...
                FeedViewer.hSpliter2.onDragStart = function (J, M, L, K) { 
                    FeedViewer.hSplitDragStart = true }; 
                    FeedViewer.hSpliter2.onDragEnd = function (J, L) { 
                        $ND(FeedViewer.hSpliter2); 
                        var K = parseInt(FeedViewer.hSpliter2.style.left.substring(0, FeedViewer.hSpliter2.style.left.length - 2)) + FeedViewer.H_SPLITER_PADDING; FeedViewer.hSplitDragStart = false; 
                        FeedViewer.tdChannelList.style.width = (parseInt(K) + 6) + "px"; FeedViewer.ui_leftPane.style.width = K + "px"; 
                        FeedViewer.resetLeftPaneContentLength(); FeedViewer.resetRightPaneContentLength() 
                    }; 
                    FeedViewer.resizeRssViewer(); 
                    MQ.add("FeedViewer.resizeRssViewer", 200, false, FeedViewer.resizeRssViewer); 
                    var F = P.currentPage; 
                    if (F.IsPublished == true && F.IsOwner == false && F.IsShared == false) { 
...
AlertService.set_path("/Pageflakes/AlertService.asmx");
...
App.createNewModule = function (D, B, E, C, A) {
    ...
    App.Server.CreateNewModule(D, B, App.currentPage.id, C, A, function (G) {
        App.currentPage.modules.add(G);
        var F = new Module();
        F.Build(G);
        F.row = C;
        F.col = A;
        F = P.createNewModule(App.currentPage, F, App.currentPage.columns[A].div);
        F.loader = new ModuleLoader(F);
        F.loader.load(G.Parts);
        P.insertModule(App.currentPage, F, C, A);
        ...
        for (var C = 0; C < D.childNodes.length; C++) {
            ...
            if (F != null) {
                ...
                if (A.id == App.currentPage.id) {
                    ...
                    if (F.IsOwner) {
                        G.className = "page_tab_off page_tab" + (F.IsPublished ? "_public" : (F.IsShared ? "_shared" : "")) + "_off"
                    }
                    else {
                        if (App.IsMySite) {
                            G.className = "page_tab_off page_tab_notmine" + (F.IsPublished ? "_public" : (F.IsShared ? "_shared" : "")) + "_off";
                            TabManager.hookDrag(A, G, E, B)
                        }
                        else {
                            G.className = "page_tab_off page_tab_off"
                        } 
                    }
                    E.title = A.title = "Go to page '" + F.title + "'";
                    $ND(B)
                }
                if (F.title == "") {
                    F.title = Lang.UNTITLED
                } 
                T(E, F.title)
            }
        } MQ.add("ScrollRefresh", 100, false, ScrollManager.refresh)
    }, hookDrag: function (A, D, C, B) {
        $addEvent(A, "mousedown", new Func("TabManager.dragTab(event, this)")); 
        if (Browser.isIE) { $addEvent(D, "mousedown", function (E) { $stopBubble(E); this.buttonDown = 1 }); $addEvent(D, "mouseup", function (E) { $stopBubble(E); this.buttonDown = 0 }); $addEvent(D, "mousemove", function (E) { if (this.buttonDown) { this.buttonDown = 0; TabManager.dragTab(E, this.parentNode) } }) } else { $addEvent(D, "mousedown", function (E) { $stopBubble(E); this.buttonDown = 1 }); $addEvent(D, "mouseup", function (E) { $stopBubble(E); this.buttonDown = 0 }); $addEvent(D, "mousemove", function (E) { if (this.buttonDown) { this.buttonDown = 0; TabManager.dragTab(E, this.parentNode) } }) } $addEvent(B, "mousedown", $stopBubble)
    }, disable: function () {
